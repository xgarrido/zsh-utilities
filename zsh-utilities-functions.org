...
#+BEGIN_SRC sh
  function __is_debian ()
  {
      if pkgtools::has_binary apt-fast; then
          ...
          __pkg_mgr="apt-get"
          return 0
      elif pkgtools::has_binary yay; then
          __pkg_mgr="yay"
          return 1
...
#+BEGIN_SRC sh
  function youtube-dl()
  {
    ...
    /tmp/youtube-dl/bin/yt-dlp --js-runtimes node $@
  }
#+END_SRC
...
#+BEGIN_SRC sh
  function mp3join()
  {
    ffmpeg -i "concat:${(j:|:)@[0,-1]}" -acodec copy joined.mp3
  }
...
#+BEGIN_SRC sh
  ...
  function upgrade-science()
  {
    ...
    pkgtools::msg_info "Upgrade following modules: ${pkgs}"
    python -m pip install --upgrade ${pkgs} $@
    pkgtools::msg_info "Upgrade python env done"
  }
  function upgrade-jupyter()
  {
    python -m pip install --upgrade jupyterlab ipywidgets plotly $@
    pkgtools::msg_info "Upgrade python env done"
  }
#+END_SRC
...
#+BEGIN_SRC sh
  ...
  {
    # pkgtools::msg_info "Starting local session of syncthing"
    # killall -q syncthing; syncthing &; syncthing browser
    pkgtools::msg_info "Starting remote session of syncthing/IJCLab..."
    
    pkgtools::msg_info "Kill remaining session"
    ssh xgarrido@ssh-ubuntu1.ijclab.in2p3.fr "killall -q syncthing"
    killall -q syncthing
  }
#+END_SRC
...
** LaTeX to unicode
#+BEGIN_SRC shell
  function latex2unicode()
  {
    python -m unicodeitplus.cli $1
  }
#+END_SRC
