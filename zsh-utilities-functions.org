  ...
  function connect ()
  {
      ...
      while [ -n "$1" ]; do
          if [[ "$1" == "-s" ]]; then
              ...
              server_name="xgarrido@cori.nersc.gov"
          elif [[ $1 == ccige* ]]; then
              ...
              server_name="osmc@xgarrido.ddns.net"
          elif [[ "$1" == "rpi" ]]; then
              server_name="ubuntu@xgarrido.ddns.net"
              ssh_option="-p 23"
          elif [[ "$1" == "lal" ]]; then
...
#+BEGIN_SRC sh
  function upgrade_venv()
  {
      ...
      while read -r line; do
          ...
          python -m pip install --upgrade ${name} | grep --line-buffered -E -v "Requirement already satisfied:|Defaulting to user"
      done <<< $(pip freeze)
