...
#+BEGIN_SRC sh
  function connect ()
  {
      ...
      if [[ "$1" == "" ]]; then
          pkgtools__msg_error "Missing the name of machine to connect !"
          __pkgtools__at_function_exit
          return 1
      fi
...
#+BEGIN_SRC sh
  function plot2pdf()
  {
      ...
      if [[ -z ${dirs} && -z ${files} ]]; then
        pkgtools__msg_notice "Adding current directory"
        dirs="."
      fi
      for d in ${=dirs}
      do
      ...
      do
          ...
          if [[ ! -a $pdf || $file -nt $pdf ]]; then
            ...
            pkgtools__msg_notice "File '$file' already processed"
          fi
      done
  }
