...
*** Wrap tikz -> pdf binary
#+BEGIN_SRC sh
  if (( $+commands[tikz2pdf] )); then
      function tikz2pdf ()
      {
          for i in $@; do
              if [ -f "$i" ]; then
                  awk '{if (!match($0,"\\draw") || !seen[$0]++) {if (match($0, "\\$\\$")) {sub("\\$", "",$0);print gensub("\\$", "", 3, $0);} else print $0}}' ${i} > ${i}.tmp
                  mv ${i}.tmp ${i}
              fi
          done
          $(pkgtools__get_binary_path tikz2pdf) $@
      }
  fi
#+END_SRC
