...
#+BEGIN_SRC sh
  function notify ()
  {
      __pkgtools__at_function_enter notify
...
#+BEGIN_SRC sh
  ...
  {
      __pkgtools__at_function_enter notify_error
      if [ $? -ne 0 ]; then
...
#+BEGIN_SRC sh
  ...
  {
      __pkgtools__at_function_enter notify_warning
      if [ $? -ne 0 ]; then
          notify -t 2000 -i stock_dialog-warning "Warning" "${PREEXEC_CMD:-Shell Command} $@"
      fi
      __pkgtools__at_function_exit
      return 0
  }
...
#+BEGIN_SRC sh
  function precmd ()
  {
      __pkgtools__at_function_enter precmd
      # must be first
      notify_error
...
$ '
  ...
  function preexec ()
  {
      __pkgtools__at_function_enter preexec
      if [[ "$TERM" == "screen" ]]; then
          local CMD=${1}
          echo -ne "\ek$CMD\e\\"
      fi
