...
about org-mode is the code wrapping that *must* be done using proper
=#+BEGIN_SRC/#+END_SRC= calls. Nevertheless, it is recommended to have a look on
org-mode features especially folding/unfolding abilities which make the read and
the navigation pretty easy.
...
** Load plugin files
The following files contain specific settings for several plugins

- Message handling and function facilities in [[zsh-utilities-pkgtools.zsh][zsh-utilities-pkgtools]]
  #+BEGIN_SRC sh
