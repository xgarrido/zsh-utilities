...
#+BEGIN_SRC sh
  if (( $+commands[ninja] )); then
      function ninja ()
      {
          $(pkgtools__get_binary_path ninja) $@ | colout -t mycmake -T ${zsh_utilities_dir} | colout -t myg++ -T ${zsh_utilities_dir} | colout -t myninja -T ${zsh_utilities_dir}
      }
  fi
#+END_SRC
*** Postamble
...
#+BEGIN_SRC sh
  ...
  {
      if [ -z $1 ]; then
          pkgtools__msg_error "Missing pdf file!"
          return 1
      fi
      local _path=$(dirname $1)
      [[ $_path = . ]] && _path=$PWD
      local _file=$(basename $1)
      docker run  -v $_path:/home/acroread/Documents:rw \
