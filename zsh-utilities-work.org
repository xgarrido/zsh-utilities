  ...
  function cmb::reorganize_mcmc()
  {
    ...
    for f in $@; do
        ...
        (
          cd ${newpath}
          if ${copy}; then
            rm -f ./mcmc.${nbr}.txt ./mcmc.updated.yaml
            cp ../$f/mcmc*.txt ./mcmc.${nbr}.txt
            cp ../$f/mcmc.updated.yaml .
            pkls=(../$f/*.pkl(N))
            for g in $pkls; do
              name=$(basename $g)
    ...
    pkgtools::msg_notice "Reorganization of directory $1 and others done"
  }
#+END_SRC
** Lyon ccali setup
*** Preamble
