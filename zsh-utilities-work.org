...
#+BEGIN_SRC sh
  ...
  alias do_aggregator_setup='aggregator setup all && mkdir -p /tmp/${USER}/snemo.d'
#+END_SRC
      ...
      if $(pkgtools__has_binary bxdpp_processing); then
          ...
          while [ -n "$1" ]; do
              ...
              if [ "${token}" = "--event-generator" -o "${token}" = "-e" ]; then
                  shift 1
                  opt+="--datatools::variant-set=core:eg=$1 "
              elif [ "${token}" = "--vertex-generator" -o "${token}" = "-v" ]; then
                  shift 1
                  opt+="--datatools::variant-set=core:vertex_generator=$1 "
              elif [ "${token}" = "--source-material" ]; then
                  shift 1
                  opt+="--datatools::variant-set=core:bb_source_material=snemo::$1 "
              elif [ "${token}" = "--bz-field" ]; then
                  shift 1
                  opt+="--datatools::variant-set=core:bz_magnitude=$1 "
              elif [ "${token}" = "--calo-energy-resolution" ]; then
                  shift 1
                  opt+="--datatools::variant-set=core:calo_energy_resolution=$1 "
              elif [ "${token}" = "--xcalo-energy-resolution" ]; then
                  shift 1
                  opt+="--datatools::variant-set=core:xcalo_energy_resolution=$1 "
              elif [ "${token}" = "--gveto-energy-resolution" ]; then
                  shift 1
                  opt+="--datatools::variant-set=core:gveto_energy_resolution=$1 "
              elif [ "${token}" = "--random-seeds" ]; then
                  opt+="--datatools::variant-set=core:random_seed_flag=true "
          ...
          bxdpp_processing \
              ...
              --datatools::resource-path=falaise@$SNAILWARE_PRO_DIR/falaise/install/share/Falaise-1.0.0/resources \
              $(echo ${opt})
      else
...
#+BEGIN_SRC shell
  ...
  function _mydpp_processing ()
  {
      ...
      _arguments                                                                                              \
          ...
          '--datatools\:\:variant-set=[Set the values of a variant]:vs:->vs'                                  \
          '*: :->args'                                                                                        \
          && ret=0
      case $state in
          log)
          ...
          eg)
              _egs=( $(sed -n '/event_generator_name/, /^\[/ { /string.enumerated_/ p }' \
                           ${_config}/snvariant_core_variants.def | awk -F \" '{print $2}') )
              _values 'Event generator' $_egs
              ;;
          vg)
              _vgs=( $(sed -n '/vertex_generator_name/, /^\[/ { /string.enumerated_/ p }' \
                           ${_config}/snvariant_core_variants.def | awk -F \" '{print $2}') )
              _values 'Vertex generator' $_vgs
              ;;
          bbsm)
              _bbsms=( $(sed -n '/bb_source_material/, /^\[/ { /string.enumerated_/ p }' \
                             ${_config}/snvariant_core_variants.def | awk -F \" '{print substr($2,8)}' ) )
              _values 'Source materials' $_bbsms
              ;;
          vs)
             _vss=( $(sed -n '/parameters\./ { s/....$//p }' \
                          ${_config}/snvariant_core_variants.def | awk -F \" '{print $2}') )
             _values 'Parameters' $_vss
             ;;
     esac
  return ret
  }
...
*** Aliases
#+BEGIN_SRC sh
  if (( $+functions[cm] )); then
      function flvisualize ()
      {
          cm $(pkgtools__get_binary_path flvisualize) $@
      }
      function bxdpp_processing ()
      {
          mkdir -p /tmp/garrido/snemo.d
          cm $(pkgtools__get_binary_path bxdpp_processing) $@
      }
  fi
  #  compdef _snemo_event_browser snemo_toto
#+END_SRC
** Lyon ccali aliases
#+BEGIN_SRC sh
