...
#+BEGIN_SRC sh
  ...
  function --bxdpp_processing ()
  {
      ...
      if (( $+commands[colout] )); then
          $(pkgtools__get_binary_path bxdpp_processing) $@ 2>&1 | \
...
#+BEGIN_SRC sh
  function mydpp_processing ()
  {
      ...
      while [ -n "$1" ]; do
          ...
          case "$token" in
              --*-generator)
                  ...
                  opt+="--datatools::variant-set=simulation:${parameter//-/_}_name=$1 "
                  ;;
              --source-material)
                  shift 1
              ...
              --*-energy-resolution | --*-*-energy-threshold)
                  shift 1
                  local parameter=${token:2}
                  opt+="--datatools::variant-set=detector:${parameter//-/_}=$1 "
                  ;;
              --random-seeds)
              ...
              --spg-*)
                  shift 1
                  local parameter=${token:2}
                  opt+="--datatools::variant-set=simulation:${parameter//-/_}=$1 "
                  ;;
              --output-path)
                  shift 1
                  if [ ! -d $1 ]; then
      ...
      _dump () {
          local header="[notice: mydpp_processing:"
          local trailer="]"
          declare -A db
          local variants=( $(echo ${opt} | tr " " "\n" | grep 'variant-set') )
          for i in ${variants}; do
              local registry=$(echo $i | awk -F'[=:]' '{print $4}')
              local parameter=$(echo $i | awk -F'[=:]' '{print $5}')
              local value=$(echo $i | awk -F'[=:]' '{print $6}')
              db[$registry]+="$header$trailer ↳ Parameter '$parameter': $value\n"
          done
          if [ ! -n "${variants}" ]; then
              echo "$header Variants set to their default values !$trailer"
          else
              echo "$header Variants dump:$trailer"
          fi

          for k in "${(@k)db}"; do
              echo "$header$trailer Registry '$k'"
              for i in ${db[$k]}; do
                  echo "$i"
              done
          done
      ...
      _falaise_version=${FALAISE_VERSION}
      --bxdpp_processing                                                                                                    \
