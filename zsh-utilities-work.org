...
#+BEGIN_SRC sh
  ...
  pkgtools::reset_variable PIP_DOWNLOAD_CACHE /tmp/garrido.d
...
#+BEGIN_SRC sh
  function run_cmb_minimize()
  {
      ...
      # Create output directory
      local minimize_dir=${parfile_dir}/${parfile_name}_${parameter_name}_min
      ...
      # Create job script
      {
          ...
          echo "else"
          echo "awk -v seed=\$RANDOM -f genrand.awk profile.par > \${current_minimize}"
          echo "fi"
          echo
      ...
      # Running job
      if ! ${dry_run}; then
          qsub -j y -P ${project} -pe multicores ${ncore} -q ${queue} -o $PWD minimize.sh
      fi
      pkgtools::at_function_exit
      return 0
  }
