...
#+BEGIN_SRC sh
  ...
  # Python 2.7
  pkgtools__add_path_to_PATH /usr/local/python/python-2.7/bin
...
#+BEGIN_SRC sh
  function run_cmb_mcmc()
  {
      ...
      if ! $(pkgtools__check_variable CAMELROOT); then
          pkgtools__msg_error "Missing CAMELROOT env. variable!"
          __pkgtools__at_function_exit
          return 1
      fi
      ...
      local mcmc_dir=${parfile_dir}/${parfile_name}_MCMC
      if [ -d ${mcmc_dir} ]; then
          pkgtools__msg_warning "Directory '${mcmc_dir}' already exist!"
          pkgtools__yesno_question "Do you want to continue ?"
          if $(pkgtools__answer_is_no); then
              __pkgtools__at_function_exit
              return 0
          fi
      fi
      mkdir -p ${mcmc_dir}; cd ${mcmc_dir}

      # Setup MCMC
      cp ${covfile} .
      # Remove precision from parfile
      grep -v precision ${parfile} > ${parfile_base}
      cat >> ${parfile_base} <<EOF
dim=$ndim
algo=ada
length=$nsample
proposal_cov=$PWD/$(basename ${covfile})
ts=10000
t0=2000
scale=0.001
do_move=false
EOF
      ...
      # Create job script
      cat > ${parfile_name}.sh << EOF
echo "running on : \$(uname -a)"
export OMP_NUM_THREADS=$ncore
cd \$TMPDIR
cp ${mcmc} .
cp $PWD/${parfile_base} mcmc\${SGE_TASK_ID}.par
cp $PWD/${covfile} .
RANDOM=\${SGE_TASK_ID}
...
./mcmc mcmc\${SGE_TASK_ID}.par $PWD/samples\${SGE_TASK_ID}.txt
...
EOF
      ...
      if $(pkgtools__answer_is_yes); then
          local qsub_args="-l sps=1 -R y -j y -o $PWD -e $PWD"
          local queue_args="-P ${project} -pe multicores ${ncore}"
          qsub ${queue_args} -q ${queue} ${qsub_args} -t 1-${nchain} -N ${parfile_name} ${parfile_name}.sh
      fi
