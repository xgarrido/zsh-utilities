...
#+BEGIN_SRC sh
  ...
  alias do_snailware_setup='aggregator setup all && mkdir -p /tmp/${USER}/snemo.d'
#+END_SRC
...
#+BEGIN_SRC sh
  function do_snemo_chain ()
  {
      ...
      if $(pkgtools__has_binary bxdpp_processing); then
          cm bxdpp_processing -m $1 -c $SNAILWARE_SIMULATION_DIR/snemo_simulation_configuration/master/module_manager.conf \
              --dlls-config $SNAILWARE_SIMULATION_DIR/snemo_simulation_configuration/master/dlls.conf \
              --datatools::resource_path=falaise@$SNAILWARE_PRO_DIR/falaise/install/share/Falaise-1.0.0/resources \
              -M 100 --modulo 1 -P notice
      else
...
#+BEGIN_SRC sh
  --mod::generate_makefile () {
      case $type in
          ...
          (article)
              echo "# -*- mode: makefile; -*-"
              echo "files_org  = $(wildcard note*.org)"
              echo "files_pdf  = doc/pdf/$(files_org:.org=.pdf)"
              echo "files_html = doc/html/$(files_org:.org=.html)"
              echo
              echo "all: pdf"
              echo
              echo "pdf: $(files_pdf)"
              echo "doc/pdf/%.pdf: %.org"
              echo "@echo \"NOTICE: Exporting $< to pdf...\""
              echo "@zsh -i -c \"org-pages --pdf --debug generate\""
              echo
              echo "html: $(files_html)"
              echo "doc/html/%.html: %.org"
              echo "@echo \"NOTICE: Exporting $< to html...\""
              echo "@zsh -i -c \"org-pages --html --debug generate\""
              echo
              echo "clean:"
              echo "@rm -rf latex.d doc *.tex *.pdf *.toc *.fdb* *~ README.el"
              echo
              echo ".PHONY: all pdf html clean"
              ;;
  }
#+END_SRC
***** Generate =.gitignore=
